---
id: version-10.2.0-preview.4-custom-scalar-types
title: Scalar Type Support
original_id: custom-scalar-types
---

Scalar types in GraphQL represent the leaf types of your graphs like `String`, `Int`.
...
We will automatically detect which of our scalars you are using and only integrate the ones you need.

This keeps your schema definition small, simple and clean.

For our built-in types we also have added automatic .NET type inference. This means that we will automatically translate for instance a `System.String` to a GraphQL `StringType`. You can override these default mappings by specifying type bindings with the schema builder.
...
The `Any scalar is a special type that can be compared to object in c#. Any allows you to specify any literal or return any type.
...
Since our field `foo` specifes an argument `bar` of type `Any` all of the following queries would be valid:
...
The same goes for the output side. Any can return a structure of data although it is still a scalar type.

If you want to access the data you can either fetch data as object or you can ask the context to provide it as a specific object.

```csharp
Foo foo = context.Argument<Foo>("bar");
```

You can also ask the context which kind the current argument is:

```csharp
ValueKind kind = context.ArgumentKind("bar");
```

The value kind will tell you by which kind of literal the argument is represented. So an integer literal can still contain a long value and a float literal could be a decimal but it also could just be a float.
...
If you want to access an object in a dynamic way without serializing it to a strongly typed model you can get it as `IReadOnlyDictionary<string, object>` or as `ObjectValueNode`.
