---
id: version-10.3.0-authorization
title: Authorization
...
If we are accessing GraphQL servers through HTTP then authenticating against a GraphQL server can be done in various ways and _Hot Chocolate_ does not prescribe any particular way.

We basically can do it in any way ASP.NET core allows we to.

So, in order to opt-in one of those solutions just add an authentication middleware to our server and our API.
